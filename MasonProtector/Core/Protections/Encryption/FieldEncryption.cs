...
namespace MasonProtector.Core
{
    internal class FieldEncryptionProtection
    {
        ...
        private const int KEY_POOL_SIZE = 8;
        ...
        private int ProtectFieldAccess(ModuleDef module, MethodDef method)
        {
            ...
            {
                if (il[i].OpCode == DnOpCodes.Ldsfld)
                {
                    ...
                    {
                        case 0:
                            postInsts.Add(Instruction.Create(DnOpCodes.Xor));
                            postInsts.Add(Instruction.Create(DnOpCodes.Ldsfld, keyFields[keyIdx]));
                            postInsts.Add(Instruction.Create(DnOpCodes.Xor));
                            break;
                        case 1:
                            postInsts.Add(Instruction.Create(DnOpCodes.Add));
                            postInsts.Add(Instruction.Create(DnOpCodes.Ldsfld, keyFields[keyIdx]));
                            postInsts.Add(Instruction.Create(DnOpCodes.Sub));
                            break;
                        case 2:
                            postInsts.Add(Instruction.Create(DnOpCodes.Ldc_I4_0));
                            postInsts.Add(Instruction.Create(DnOpCodes.Mul));
                            postInsts.Add(Instruction.Create(DnOpCodes.Add));
                            break;
                    }
