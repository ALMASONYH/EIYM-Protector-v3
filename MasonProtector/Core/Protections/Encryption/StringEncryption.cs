...
namespace MasonProtector.Core
{
    internal class StringEncryptionProtection
    {
        ...
        private const int VAULT_COUNT = 12;
        private const int VAULT_SIZE = 320;
        private const int DECRYPTOR_COUNT = 18;
        ...
        internal void ApplyStringEncryption(ModuleDef module, TypeDef modType)
        {
            ...
            for (int v = 0; v < VAULT_COUNT; v++)
            {
                ...
                for (int r = 0; r < 2; r++)
                {
                    var decoy = BuildVaultResolver(module, field, engine.MakeName(), rng.Next(0, 3));
        ...
        private void EncryptMethodStrings(ModuleDef module, TypeDef modType, MethodDef method)
        {
            ...
            for (int i = 0; i < il.Count; i++)
            {
                ...
                if (string.IsNullOrEmpty(orig) || orig.Length > 800) continue;

                int mode = rng.Next(0, 6);
                int key1 = rng.Next(1, 255);
                int key2 = rng.Next(1, 255);
                while (key2 == key1) key2 = rng.Next(1, 255);
                ...
                int copyCount = strDecryptors.Count / 6;
                if (copyCount > 1)
                    decryptorIdx = mode + 6 * rng.Next(0, copyCount);
        ...
        private List<Instruction> BuildSameVaultRetrieval(int target)
        {
            ...
            switch (rt)
            {
                case 0: vaultData[v][s2] = vaultData[v][s1] ^ target; break;
                case 1: vaultData[v][s2] = target - vaultData[v][s1]; break;
                case 2: vaultData[v][s2] = ~vaultData[v][s1] ^ target; break;
            }
        ...
        private List<Instruction> BuildCrossVaultRetrieval(int target)
        {
            ...
            int rtA = rng.Next(0, 3);
            switch (rtA)
            {
                case 0: vaultData[vA][a2] = vaultData[vA][a1] ^ partial; break;
                case 1: vaultData[vA][a2] = partial - vaultData[vA][a1]; break;
                case 2: vaultData[vA][a2] = ~vaultData[vA][a1] ^ partial; break;
            }

            int rtB = rng.Next(0, 3);
            switch (rtB)
            {
                case 0: vaultData[vB][b2] = vaultData[vB][b1] ^ other; break;
                case 1: vaultData[vB][b2] = other - vaultData[vB][b1]; break;
                case 2: vaultData[vB][b2] = ~vaultData[vB][b1] ^ other; break;
            }
        ...
        private List<Instruction> BuildTripleVaultRetrieval(int target)
        {
            ...
            int rtA = rng.Next(0, 3), rtB = rng.Next(0, 3), rtC = rng.Next(0, 3);
        ...
        private void SetVaultPair(int vault, int s1, int s2, int target, int rt)
        {
            switch (rt)
            {
                case 0: vaultData[vault][s2] = vaultData[vault][s1] ^ target; break;
                case 1: vaultData[vault][s2] = target - vaultData[vault][s1]; break;
                case 2: vaultData[vault][s2] = ~vaultData[vault][s1] ^ target; break;
            }
        }
        ...
        private MethodDefUser BuildDecryptor(ModuleDef module, string name, int mode)
        {
            ...
            var loopEnd = Instruction.Create(DnOpCodes.Ldloc, method.Body.Variables[5]);
