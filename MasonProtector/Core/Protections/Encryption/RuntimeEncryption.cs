...
namespace MasonProtector.Core
{
    internal class RuntimeEncryptionProtection
    {
        ...
        internal void ApplyResourceProtection(ModuleDef module, TypeDef modType)
        {
            ...
            initIl.Add(engine.LoadInt(encryptedNames.Count));
            ...
            for (int i = 0; i < encryptedNames.Count; i++)
            {
                ...
                initIl.Add(engine.LoadInt(i));
                initIl.Add(Instruction.Create(DnOpCodes.Ldstr, encryptedNames[i]));
                initIl.Add(Instruction.Create(DnOpCodes.Stelem_Ref));
            }
            initIl.Add(Instruction.Create(DnOpCodes.Ret));
            ...
            if (hasAnyResourcesFile)
            {
                try
                {
                    MethodDef customRMCtor = BuildCustomResourceManager(module, helperMethod);
                    RewriteResourceManagerCtorCalls(module, customRMCtor);
                }
                catch { }
        ...
        private MethodDef BuildCustomResourceManager(ModuleDef module, MethodDef helperMethod)
        {
        ...
        private void RewriteResourceManagerCtorCalls(ModuleDef module, MethodDef customRMCtor)
        {
            foreach (TypeDef type in module.GetTypes())
            {
                ...
                foreach (MethodDef method in type.Methods)
                {
                    ...
                    for (int i = 0; i < ins.Count; i++)
                    {
                        ...
                        if (sig == null || sig.Params.Count != 2) continue;
                        if (sig.Params[0].FullName != "System.String") continue;
                        if (sig.Params[1].FullName != "System.Reflection.Assembly") continue;
                        instr.Operand = customRMCtor;
                    }
                }
            }
        }
