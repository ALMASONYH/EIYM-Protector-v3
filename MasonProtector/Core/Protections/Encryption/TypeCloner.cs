...
namespace MasonProtector.Core
{
    internal class TypeCloner
    {
        ...
        private TypeDef TryRemapTypeByName(ITypeDefOrRef tOp)
        {
            if (tOp == null) return null;
            string fn = tOp.FullName;
            foreach (var kv in typeMap)
            {
                if (kv.Key.FullName == fn) return kv.Value;
            }
            return null;
        }

        private MethodDef TryRemapMethodByName(IMethod mOp)
        {
            if (mOp == null || mOp.DeclaringType == null) return null;
            string declName = mOp.DeclaringType.FullName;
            string mName = mOp.Name;
            foreach (var kv in methodMap)
            {
                MethodDef src = kv.Key;
                if (src.DeclaringType == null) continue;
                if (src.DeclaringType.FullName != declName) continue;
                if (src.Name != mName) continue;
                if (mOp.MethodSig != null && src.MethodSig != null &&
                    mOp.MethodSig.Params.Count != src.MethodSig.Params.Count)
                    continue;
                return kv.Value;
            }
            return null;
        ...
        private FieldDef TryRemapFieldByName(IField fOp)
        {
            if (fOp == null || fOp.DeclaringType == null) return null;
            string declName = fOp.DeclaringType.FullName;
            string fName = fOp.Name;
            foreach (var kv in fieldMap)
            {
                FieldDef src = kv.Key;
                if (src.DeclaringType == null) continue;
                if (src.DeclaringType.FullName != declName) continue;
                if (src.Name != fName) continue;
                return kv.Value;
            }
            return null;
        }
