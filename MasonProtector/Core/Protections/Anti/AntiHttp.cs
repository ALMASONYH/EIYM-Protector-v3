...
namespace MasonProtector.Core
{
    internal class AntiHttpProtection
    {
        ...
        internal void ApplyAntiHttp(ModuleDef module, TypeDef modType)
        {
            ...
            var webProxyCtor = module.Import(typeof(System.Net.WebProxy).GetConstructor(Type.EmptyTypes));
            var setDefaultProxy = module.Import(typeof(System.Net.WebRequest).GetProperty("DefaultWebProxy").GetSetMethod());
            var setConnLimit = module.Import(typeof(System.Net.ServicePointManager).GetProperty("DefaultConnectionLimit").GetSetMethod());
            ...
            var tryStart = Instruction.Create(DnOpCodes.Newobj, webProxyCtor);
            il.Add(tryStart);
            il.Add(Instruction.Create(DnOpCodes.Call, setDefaultProxy));

            il.Add(Instruction.Create(DnOpCodes.Ldc_I4, rng.Next(1, 4)));
            il.Add(Instruction.Create(DnOpCodes.Call, setConnLimit));

            il.Add(Instruction.Create(DnOpCodes.Ldc_I4, 3072 | 12288));
