...
namespace MasonProtector.Core
{
    internal class AntiDumpProtection
    {
        ...
        internal void ApplyAntiDump(ModuleDef module, TypeDef modType)
        {
            ...
            ModuleRef kernel32 = null;
            foreach (var mr in module.GetModuleRefs())
            {
                if (mr.Name == "kernel32.dll") { kernel32 = mr; break; }
            }
            if (kernel32 == null)
            {
                kernel32 = new ModuleRefUser(module, "kernel32.dll");
            }

            var vpMethod = new MethodDefUser(engine.MakeName(),
                ...
                DnMethodAttributes.PinvokeImpl | DnMethodAttributes.Private |
                DnMethodAttributes.Static | DnMethodAttributes.HideBySig);
            vpMethod.ImplMap = new ImplMapUser(
                kernel32,
                new UTF8String("VirtualProtect"),
                PInvokeAttributes.CallConvWinapi);
            dumpType.Methods.Add(vpMethod);
